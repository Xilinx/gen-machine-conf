...
def read_packages(xilinx_arch):
    ...
    for line in Lines_config:
        ...
        if re.search("#|\n", line_str):
            ...
            packages['new_groups'] = re.sub(r'"|\]|\\' , r'',new_groups).rstrip("'")
...
def add_user_params(packages,bb_file):
    ...
    if new_groups != "":
        for group in new_groups.split(";"):
            if group: user_params += 'groupadd -r %s;' % (group)
...
def parse_args(argv):
    try:
        opts, args = getopt.getopt(argv,"hk:b:",["generate_kconfig=","update_cfg="])
    ...
    for opt, arg in opts:
        if opt == '-h':
            print('HELP: ')
            print("Usage: python3 rootfs_config.py --generate_kconfig <user_rootfsconfig> <Kconfig Out path>")
            print("Usage: python3 rootfs_config.py --update_cfg config <cfg_file> <xilinx_arch>")
            sys.exit()
            ...
            print("Error:")
            print("Usage: python3 rootfs_config.py --generate_kconfig <user_rootfsconfig> <Kconfig Out path>")
            print("Usage: python3 rootfs_config.py --generate_bb config <cfg_file> <xilinx_arch>")
